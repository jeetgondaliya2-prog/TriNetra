...
import liveFeedVideo from "@/assets/live-feed.mp4.asset.json";
...
export function LiveCanvas({
  ...
  return (
    <div className="relative aspect-video w-full overflow-hidden rounded-lg border border-border bg-black">
      <video
        ref={videoRef}
        // local copy shipped in public/ (works offline in VS Code); falls back to CDN copy
        src="/live-feed.mp4"
        onError={(e) => {
          const el = e.currentTarget;
          if (!el.src.endsWith(liveFeedVideo.url)) {
            el.src = liveFeedVideo.url;
            el.load();
