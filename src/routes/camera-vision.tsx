...
import { Flame, Pause, Play, RotateCcw, Siren, Upload, Video, Webcam } from "lucide-react";
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
        <div className="space-y-4">
          ...
          <section className="panel p-4">
            <h3 className="mb-3 text-sm font-semibold">Stream sources</h3>
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                { icon: Upload, t: "Drag & drop upload", s: "mp4 / mov / avi → /storage/uploads" },
                ...
                <div
                  key={t}
                  className="rounded-md border border-dashed border-border px-4 py-5 text-center"
                >
                  <Icon className="mx-auto size-5 text-primary" />
                  <p className="mt-2 text-sm font-medium">{t}</p>
                  <p className="mt-1 font-mono text-[11px] text-muted-foreground">{s}</p>
                </div>
              ))}
            </div>
          </section>
